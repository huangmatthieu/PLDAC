				...
				for(int j = 0 ; j < nbMod ; j++){ //Pour chaque modalit̩ k de j
					if ( cardSimple[i][j] / nbLigData > FFS.params.getSupp()){ //si support suffisant
						...
						if( t > FFS.params.getTs()){
							QPSummary qps = new QPSummary(q.get(x),vocab.getLVInCol(i),j,t,verbBe,verbHave,entities);
					...
					for(int k2  = 0 ; k2 < nbMod2 ; k2++){ //Pour chaque modalit̩ k'de j'
						...
						for(int q1 = 0 ; q1 < tailleQ ; q1++){ //Pour chaque quantifier q
							if(q.get(q1).getType().equals(QType.ABSOLUTE)){  //si q est absolu
								...
								if(t > FFS.params.getTs()){
									QRPSummary qrp = new QRPSummary(q.get(q1),vocab.getLVInCol(j),vocab.getLVInCol(i),k2,k1,t,verbBe,verbHave,entities);
								...
								if(cardSimple[j][k2] / nbLigData > FFS.params.getSupp()){
									...
									if(t > FFS.params.getTs()){
										QRPSummary qrp = new QRPSummary(q.get(q1),vocab.getLVInCol(i),vocab.getLVInCol(j),k1,k2,t,verbBe,verbHave,entities);
										sum.getQRPSummaries().add(qrp);
									}
								}
							}
