...
public class Ruspini extends FuzzyPart {
	...
	public MShips mShip(double x) {
		...
		if (x < keyPoints.get(0))  // si x < première modalté
			...
			leftId = 1;
			rightId = 2 * (m - 1);
			i = (int) Math.floor((rightId - leftId) / 2) + 1;
	
			while ((x < keyPoints.get(i - 1)) || (x > keyPoints.get(i))) {
				if (x <= keyPoints.get(i - 1)) rightId = i;
				else leftId = i;
				i = (int) Math.floor(((rightId - leftId) / 2)) + leftId;
			}
	
			if ((i % 2) == 0) return result.fill1Mod((i / 2) + 1, 1);
			else return result.fill2Mod(((i+1) / 2), (keyPoints.get(i) - x) / (keyPoints.get(i) - keyPoints.get(i-1)));
		}
	}
}
